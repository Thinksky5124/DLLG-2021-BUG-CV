...
public:
    ...
    std::vector<float> p4pSolution(const std::vector<cv::Point2f> objectPoints,int objectType);
...
private:
    ...
    cv::Mat rVec = cv::Mat::zeros(3, 1, CV_32FC1);//旋转向量
    cv::Mat tVec = cv::Mat::zeros(3, 1, CV_32FC1);//平移矩阵
